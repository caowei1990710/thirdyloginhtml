(function () {
  ...
  function thirdyRplistController($scope, $timeout, bankcard, $rootScope, $state, httpRequest, $filter) {
    ...
    vm.get = function (page) {
      if (vm.query.state == "" && vm.query.wechatName == "" && vm.query.depositNumber == "" && vm.query.ip == "" && vm.query.payType == "" && vm.query.beginTime == vm.defaultbegin && vm.query.endTime == vm.defaultend) {
        ...
        httpRequest.get("deposit?page=" + page + "&userRemark=" + vm.query.userRemark + "&payType=" + vm.query.payType + "&size=" + vm.sizePerPage + "&state=" + vm.query.state + "&beginTime=0&endTime=0&beginTranstime=" + new Date(vm.query.beginTime).getTime() + "&endTranstime=" + new Date(vm.query.endTime).getTime() + "&beginExcuteTime=0&endExcuteTime=0&wechatName=" + vm.query.wechatName + "&depositNumber=" + vm.query.depositNumber + "&billNo=&platfrom=" + vm.query.platfrom + "&ip=" + vm.query.ip, vm.getDate)
      }
    }
