(function () {
  ...
  function thirdyProposalController($scope, httpRequest, $filter, $rootScope, $timeout) {
    ...
    vm.result = function (data) {
      ...
      vm.outList = function () {
        var url = "exportcashout?page=1&state=" + vm.query.state + "&platfrom=" + ((vm.query.platfrom == undefined) ? "" : vm.query.platfrom) + "&size=10&bankDest=" + ((vm.query.bankDest == undefined) ? "" : vm.query.bankDest) + "&beginTime=" + new Date(vm.query.beginTime).getTime() + "&endTime=" + new Date(vm.query.endTime).getTime();
    ...
    vm.getPayone = function () {
      httpRequest.get("wechat?name=" + vm.payitem.payoneBankcard + "&platfrom=&type=0&state=&page=1&size=10", vm.getOneBankCard)
    }
    vm.getPaytwo = function () {
      httpRequest.get("wechat?name=" + vm.payitemtwo.paytwoBankcard + "&platfrom=&type=0&state=&page=1&size=10", vm.getTwoBankCard)
    }
    ...
    vm.get = function (page) {
      ...
      httpRequest.get("cashoutproposal" + "?page=" + page + "&state=" + vm.query.state + "&platfrom=" + ((vm.query.platfrom == undefined) ? "" : vm.query.platfrom) + "&size=10&bankDest=" + ((vm.query.bankDest == undefined) ? "" : vm.query.bankDest) + "&beginTime=" + new Date(vm.query.beginTime).getTime() + "&endTime=" + new Date(vm.query.endTime).getTime(), vm.result)
    }
  }
})();
