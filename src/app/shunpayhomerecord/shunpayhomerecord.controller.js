(function () {
  ...
  function shunpayHomeRecordController($scope, $timeout, httpRequest, $rootScope, $state, loc_language_cn, $filter) {
    var vm = this;
    vm.query = {"state": "", "wechatName": "", "depositNumber": ""};
    vm.sizePerPage = 500;
    vm.page = 1;
    vm.totalSum = 0;
    vm.nowSum = 0;
    ...
    vm.get = function (page) {
      ...
      if ($rootScope.user.callbackurl == 'www.baidu.com') {
        ...
        httpRequest.get("deposit?page=" + page + "&size=500&state=" + vm.query.state + "&beginTime=0&endTime=0&beginTranstime=" + new Date(vm.query.beginTime).getTime() + "&endTranstime=" + new Date(vm.query.endTime).getTime() + "&beginExcuteTime=0&endExcuteTime=0&platfrom=" + $rootScope.user.id + "&wechatName=" + vm.query.wechatName + "&depositNumber=" + vm.query.depositNumber + "&billNo=", vm.getDate)
    }
  }
})();
